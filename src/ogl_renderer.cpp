...
static constexpr std::string_view POS_HEADER = "v ";
static constexpr std::string_view NORMAL_HEADER = "vn ";
...
static usize
obj_vertex_get_idx(const std::vector<Vertex>& vertices, const Vertex& vertex) {
...
Mesh::from_obj(const char* path, Error* err) {
  ...
  std::string file = platform::read_entire_file(path);
  ...
  while (std::getline(file_stream, line)) {
    if (line.starts_with(POS_HEADER)) ++pos_count;
    else if (line.starts_with(NORMAL_HEADER)) ++normal_count;
    else if (line.starts_with(UV_HEADER)) ++uv_count;
    else if (line.starts_with(INDEX_HEADER)) ++index_count;
  }
  index_count *= 3;
  ...
  while (std::getline(file_stream, line)) {
    if (line.starts_with(INDEX_HEADER)) {
      std::size_t pos{0};
      ...
      std::string part{line.substr(pos, space_idx - pos)};
      ...
      ERROR_ASSERT(error == Error::Success, *err, error, {});
      obj_vertex_push_if_missing_and_push_idx(vertices, indices, vertex);
    } else if (line.starts_with(POS_HEADER)) {
      std::size_t pos{0};
      std::size_t space_idx{line.find(' ', pos)};
      ERROR_ASSERT(space_idx != std::string::npos, *err, Error::ObjInvalidData, {});
      pos = space_idx + 1;
    ...
    } else if (line.starts_with(NORMAL_HEADER)) {
      std::size_t pos{0};
      std::size_t space_idx{line.find(' ', pos)};
      ERROR_ASSERT(space_idx != std::string::npos, *err, Error::ObjInvalidData, {});
      pos = space_idx + 1;
    ...
    } else if (line.starts_with(UV_HEADER)) {
      std::size_t pos{0};
      std::size_t space_idx{line.find(' ', pos)};
      ERROR_ASSERT(space_idx != std::string::npos, *err, Error::ObjInvalidData, {});
      pos = space_idx + 1;
      ...
      uvs.push_back({u, v});
    } else if (line.starts_with(MTL_FILE_HEADER)) {
      usize space_idx{line.find(' ')};
      std::string material_file_path{line.substr(space_idx + 1)};
...
static u32
setup_shader(const std::filesystem::path& path, ShaderType shader_type, Error* err) {
  std::string shader_src = platform::read_entire_file(path);
  ...
  const char* shader_data = shader_src.data();
...
Texture::Texture(const image::Image& img) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, static_cast<GLsizei>(img.width),
               static_cast<GLsizei>(img.height), 0, GL_RGBA, GL_UNSIGNED_BYTE, img.data.data());
